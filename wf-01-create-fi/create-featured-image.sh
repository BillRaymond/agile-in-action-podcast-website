...
echo “* START Avoid traps along the value trail”
convert fi-template.png `# load template background image`\
\( ../uploads/wf-guest-images-fi/david-pereira-v2.png -resize 282x282 -set page +256+205 \) `# load a guest photo`\
...
-size 580x340 caption:'Avoid traps along the value trail' `# Podcast title as it appears on the website`\
...
-size 405x195 caption:'with David Pereira\nCEO, omoqo GmbH and Author of Untrapping Product Teams' `# List the guest names for the podcast`\
-geometry +550+446 `# Set the x and y position of the guest names`\
-composite `# Add guest names to the image`\
+write ../uploads/wf-featured-images/2024-05-28-avoid-traps-along-the-value-trail-no-play.png \
-page +972+448 fi-play-icon-youtube.png `# load YouTube icon image`\
-layers flatten \
../uploads/wf-featured-images/2024-05-28-avoid-traps-along-the-value-trail.png

echo “* FINISH Avoid traps along the value trail”

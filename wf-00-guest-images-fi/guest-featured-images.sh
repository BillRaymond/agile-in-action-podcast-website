...
set -e -x
echo “* START Converting guest photo: uploads/dr-tan-trung-john-luong.jpeg”
...
echo “* FINISH converted guest photo to: uploads/wf-guest-images-fi/daria-bagina.png”echo “* START Converting guest photo: uploads/idzik_img.png”
convert uploads/idzik_img.png `# load the guest’s photo`\
    -resize 250x250^ `# load template background image`\
    -gravity center `# Set the center of gravity for the photo`\
    -background transparent `# Background should be transparent`\
    -extent 250x250 `# Crop the photo to a perfect square`\
    ...
    \( +clone -background black -shadow 60x5+6+6 \) `# Create a shadow under the photo`\
    +swap -background none -layers merge +repage `# Merge the photo and shadow layers`\
    uploads/wf-guest-images-fi/idzik_img.png `# Save the guest photo`\

echo “* FINISH converted guest photo to: uploads/wf-guest-images-fi/idzik_img.png”